...
File.prototype.query = function (options, callback) {
  ...
  if (options.query) {
    //  currently support queries of the syntax:
    //    json.[fieldName]:[fieldValue]
    var parts = options.query.split('.');
    if (parts && parts.length === 2 && parts[0].toLowerCase() === 'json') {
  ...
  function check(log) {
    ...
    if (options.parsedQuery) {
      if (!log[options.parsedQuery.fieldName] || log[options.parsedQuery.fieldName] !== options.parsedQuery.fieldValue) {
        return;
      }
    }
