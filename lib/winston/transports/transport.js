...
Transport.prototype.normalizeQuery = function (options) {
  ...
  // limit
  if (options.rows === 0 || options.limit === 0) {
    //  if options.rows or options.limit is explicity set to 0 by the caller,
    //  retain this value (which signifies no limit on rows returned)
    options.rows = 0;
  }
  else {
    //  if no explicit value for options.rows or options.limit, then
